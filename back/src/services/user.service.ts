...
class userService {
    ...
    async updateUsername(userId: string, name: string) {
        const findByUserId = await userController.findByUserId(userId);
        if (findByUserId[0].name !== name) {
            const findByName = await userController.findByName(name);
            if (findByName.length !== 0) {
                const errorMessage: string = "이미 존재하는 이름입니다.";
                return errorMessage;
            }
    ...
    async userWithdrawal(userId: string, id: string, withdrawal: number) {
        ...
        if (withdrawal === 1) {
            ...
            const username = "withdrawn "+uuidv4().split("-",1);
